class YgtPreEms3OrgImgState < ActiveRecord::Base
  establish_connection :ygtdb
  self.primary_keys = :ems_no, :cop_g_no
  self.table_name = 'sap.pre_ems3_org_img_state'

  def self.upload_sap
    rows = YgtPreEms3OrgImgState.where(rstat: '1')
    YgtPreEms3OrgImgState.transaction do
